...
public class ExpertGmActivity extends AppCompatActivity {
    private final Random rnd = new Random();
    private static int[] randomAnimals = {0, 0, 0, 0, 0, 0};
    private static int[] randomMixAnimals = {0, 0, 0, 0, 0, 0};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // копия массива для перемешивания
        for (int i = 0; i < 6; i++) {
            randomMixAnimals[i] = randomAnimals[i];
        }

        // прячем фото через 5 сек
        ...
        new CountDownTimer(TIME, 1000) {
            public void onTick(long millisUntilFinished) {
                ...
                // перемешивание фото
                for (int i = 0; i < 20; i++) {
                    int i1 = rnd.nextInt(S);
                    int i2 = rnd.nextInt(S);
                    int tmp = randomMixAnimals[i1];
                    randomMixAnimals[i1] = randomMixAnimals[i2];
                    randomMixAnimals[i2] = tmp;
                }

                pic1.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[0], "drawable", getApplicationContext())));
                pic2.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[1], "drawable", getApplicationContext())));
                pic3.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[2], "drawable", getApplicationContext())));
                pic4.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[3], "drawable", getApplicationContext())));
                pic5.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[4], "drawable", getApplicationContext())));
                pic6.setImageDrawable(getResources().getDrawable(getResourceID("a" + randomMixAnimals[5], "drawable", getApplicationContext())));
